...
lg_handle = aq.get("GEAR HANDLE POSITION")
lg_percent = aq.get("GEAR TOTAL PCT EXTENDED")

#set up the leds on the launchpad for the gear, if handle is off, show red led 

if lg_handle == 0:
    lp.ledCtrlXY(4, 3, 64, 64, 64) #gear handle up position light

elif lg_handle == 1:
    lp.ledCtrlXY(4, 4, 64, 64, 64) #gear handle down position light
...
ab_pos = aq.get("AUTOBRAKE SWITCH POSITION")
...
flaps_pos = aq.get("FLAPS HANDLE INDEX") #from 0 to 4, off, 1, 2 ,3, full
